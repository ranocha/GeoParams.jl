...
function ArrheniusType2(args...)
    return ArrheniusType2(
        convert(GeoUnit, args[1]),
        convert(GeoUnit, args[2]),
        convert(GeoUnit, args[3]),
        convert(GeoUnit, args[4]),
        convert(GeoUnit, args[5]),
        args[6],
    )
end

function param_info(a::ArrheniusType2) # info about the struct
    return MaterialParamsInfo(;
        Equation=L"\tau_{ij} = 2 \eta_0 * \exp((E_a + P V_a) / (R T) - E_a / (R * T_0))  \dot{\varepsilon}_{ij}",
    )
end

# Calculation routines for linear viscous rheologies
function compute_εII(
    a::ArrheniusType2, TauII::_T; P=0.0, T=273.0, depth=0.0, kwargs...
) where {_T}
...
"""
    compute_εII!(EpsII::AbstractArray{_T,N}, s::ArrheniusType2, TauII::AbstractArray{_T,N})
"""
function compute_εII!(
    EpsII::AbstractArray{_T,N},
    a::ArrheniusType2,
    TauII::AbstractArray{_T,N};
    P=zeros(size(TauII))::AbstractArray{_T,N},
    depth=zeros(size(TauII))::AbstractArray{_T,N},
    T=fill(273.0, size(TauII))::AbstractArray{_T,N},
    kwargs...,
) where {N,_T}
    @inbounds for i in eachindex(EpsII)
        TauII[i] = compute_τII(a, EpsII[i]; P=P[i], T=T[i], depth=depth[i])
    end
...
"""
    compute_τII(s::ArrheniusType2, EpsII; kwargs...)

Returns second invariant of the stress tensor given a 2nd invariant of strain rate tensor 
"""
function compute_τII(
    a::ArrheniusType2, EpsII::_T; P=0.0, T=273.0, depth=0.0, kwargs...
) where {_T}
    @unpack_val η0, Ea, Va, T0, R = a
    η = η0 * exp((Ea + P * Va) / (R * T) - Ea / (R * T0))
    # correction = (depth ≤ 660e3) + (depth > 660e3) * 1e1 
...
# Print info 
function show(io::IO, g::ArrheniusType2)
    return print(
        io,
        "ArrheniusType2: η0 = $(Value(g.η0)), Ea = $(Value(g.Ea)), Va = $(Value(g.Va)), T0 = $(Value(g.T0)), R = $(Value(g.R))",
    )
end
# ------------------------------------------------------------------------
